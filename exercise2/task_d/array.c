...
Array array_new(long capacity, double realloc_factor){
    assert(capacity > 0);
    return (Array){malloc(sizeof(long)*capacity), 0, 0, capacity, 10*1000*1000, realloc_factor, 0}; //10 Mb
}
...
void array_reserve(Array* a, long capacity){
  // TODO: your code here
  a->n_reallocations++;
  if (capacity >= a->upper_limit)
    printf("Upper limit reached after %d calls\n", a->n_reallocations);
  long* peg = a->data;
  a->data = malloc(sizeof(long)*capacity);
  memcpy(a->data, peg, a->capacity*sizeof(long));
  free(peg);
...
void array_insertBack(Array* a, long stuff){
  ...
  if (a->back >=  a->capacity)
    array_reserve(a, a->capacity*a->realloc_factor);
  a->data[a->back] = stuff;
  a->back++;
}
